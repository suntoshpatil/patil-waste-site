...
export async function POST(req: Request) {
  try {
    const { customer, planName, firstPickup, invoiceTotal } = await req.json()
    if (!process.env.RESEND_API_KEY || process.env.RESEND_API_KEY === 're_placeholder') return NextResponse.json({ ok: true, skipped: true })
    await resend.emails.send(contractAcceptedEmail(customer, planName, firstPickup, invoiceTotal || 0))
