...
export async function POST(req: Request) {
  try {
    const { customerId, planName, pickupDay, startDate } = await req.json()
    ...
    await resend.emails.send(contractReadyEmail(customer, planName || 'Service Plan', pickupDay || '', startDate || ''))
